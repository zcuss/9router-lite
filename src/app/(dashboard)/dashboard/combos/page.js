      ...
      {combos.length === 0 ? (
        <Card>
          <div className="text-center py-12">
            ...
            <p className="text-sm text-[var(--color-text-muted)] mb-4">Create model combos with fallback support</p>
            <Button icon="add" onClick={() => setShowCreateModal(true)} className="w-full sm:w-auto">
              Create Combo
            </Button>
          </div>
    ...
    <Card padding="sm" className="group">
      <div className="flex min-w-0 flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
        <div className="flex min-w-0 flex-1 items-start gap-3 sm:items-center">
          <div className="size-8 rounded-lg bg-[var(--color-text-main)]/10 flex items-center justify-center shrink-0">
            <span className="material-symbols-outlined text-[var(--color-text-main)] text-[18px]">layers</span>
          </div>
          <div className="min-w-0 flex-1">
            <code className="block truncate font-mono text-sm font-medium">{combo.name}</code>
            <div className="mt-1 flex min-w-0 flex-wrap items-center gap-1">
              {combo.models.length === 0 ? (
                <span className="text-xs text-[var(--color-text-muted)] italic">No models</span>
              ) : (
                combo.models.slice(0, 3).map((model, index) => (
                  <code key={index} className="max-w-full truncate rounded bg-[var(--color-surface-2)] px-1.5 py-0.5 font-mono text-[10px] text-[var(--color-text-muted)] dark:bg-[var(--color-surface)]/5 sm:max-w-[220px]">
                    {model}
                  </code>
                ))
              )}
        ...
        <div className="flex w-full flex-col gap-2 sm:w-auto sm:flex-row sm:items-center sm:gap-3 sm:shrink-0">
          ...
          <div className="flex items-center justify-between gap-1.5 rounded-lg bg-[var(--color-surface-2)] px-2 py-1.5 dark:bg-[var(--color-surface)]/[0.02] sm:justify-start sm:bg-transparent sm:px-0 sm:py-0 sm:dark:bg-transparent">
          ...
          <div className="grid grid-cols-3 gap-1 sm:flex">
            <button
              ...
              className="flex flex-col items-center rounded px-2 py-1 text-[var(--color-text-muted)] transition-colors hover:bg-[var(--color-surface-2)] hover:text-[var(--color-text-main)] dark:hover:bg-[var(--color-surface)]"
            ...
            <button
              onClick={onDelete}
              className="flex flex-col items-center rounded px-2 py-1 text-[var(--color-danger)] transition-colors hover:bg-[var(--color-danger)]/10"
              title="Delete"
            >
...
function ModelItem({ id, index, model, isFirst, isLast, onEdit, onMoveUp, onMoveDown, onRemove }) {
  ...
  return (
    <div
      ref={setNodeRef}
      style={style}
      className={`group flex min-w-0 items-center gap-1.5 rounded-md px-2 py-1 bg-[var(--color-surface-2)] hover:bg-[var(--color-surface-2)] dark:bg-[var(--color-surface)]/[0.02] dark:hover:bg-[var(--color-surface)]/[0.04] transition-colors ${isDragging ? "shadow-md ring-1 ring-primary/30" : ""}`}
    >
      {/* Drag handle */}
      <button
        {...attributes}
        {...listeners}
        type="button"
        className="cursor-grab touch-none p-0.5 rounded text-[var(--color-text-muted)] hover:text-[var(--color-text-main)] active:cursor-grabbing shrink-0"
      ...
      {editing ? (
        ...
        <div
          className="min-w-0 flex-1 cursor-text truncate rounded px-1.5 py-0.5 font-mono text-xs text-[var(--color-text-main)] hover:bg-[var(--color-surface-2)] dark:hover:bg-[var(--color-surface)]"
      ...
      <div className="flex shrink-0 items-center gap-0.5">
        <button
          onClick={onMoveUp}
          disabled={isFirst}
          className={`p-0.5 rounded ${isFirst ? "text-[var(--color-text-muted)]/20 cursor-not-allowed" : "text-[var(--color-text-muted)] hover:text-[var(--color-text-main)] hover:bg-[var(--color-surface-2)] dark:hover:bg-[var(--color-surface)]"}`}
          title="Move up"
        >
        ...
        <button
          onClick={onMoveDown}
          disabled={isLast}
          className={`p-0.5 rounded ${isLast ? "text-[var(--color-text-muted)]/20 cursor-not-allowed" : "text-[var(--color-text-muted)] hover:text-[var(--color-text-main)] hover:bg-[var(--color-surface-2)] dark:hover:bg-[var(--color-surface)]"}`}
          title="Move down"
        >
