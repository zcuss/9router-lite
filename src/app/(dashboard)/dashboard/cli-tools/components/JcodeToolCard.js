              ...
              <div className="flex flex-col gap-3 p-4 bg-[var(--color-accent)]/10 border border-[var(--color-accent)]/30 rounded-lg">
                <div className="flex items-start gap-3">
                  ...
                  <div className="flex-1">
                    <p className="font-medium text-[var(--color-accent)] dark:text-[var(--color-accent)]">jcode CLI not detected locally</p>
                    <p className="text-sm text-[var(--color-text-muted)] mt-1">Install jcode to enable automatic configuration:</p>
                    <code className="block mt-2 p-2 bg-[var(--color-surface-2)]/20 rounded text-xs font-mono">
                      curl -fsSL https://raw.githubusercontent.com/1jehuang/jcode/master/scripts/install.sh | bash
                    </code>
                    <p className="text-sm text-[var(--color-text-muted)] mt-2">Manual configuration is still available if 9router is deployed on a remote server.</p>
                  </div>
                </div>
          ...
          {!checkingJcode && jcodeStatus?.installed && (
            <>
              <div className="flex flex-col gap-2">
                  ...
                  <div className="flex flex-col gap-2 mb-2">
                    {tool.notes.map((note, idx) => (
                      <div key={idx} className={`flex items-start gap-2 p-2 rounded text-xs ${
                        note.type === "info" ? "bg-[var(--color-accent)]/10 text-[var(--color-accent)] dark:text-[var(--color-accent)]" :
                        note.type === "warning" ? "bg-[var(--color-accent)]/10 text-[var(--color-accent)] dark:text-[var(--color-accent)]" :
                ...
                {/* Default Model */}
                <div className="grid grid-cols-1 gap-1.5 sm:grid-cols-[8rem_auto_1fr_auto] sm:items-center sm:gap-2">
                  <span className="text-xs font-semibold text-[var(--color-text-main)] sm:text-right sm:text-sm">Default Model</span>
                  <span className="material-symbols-outlined hidden text-[var(--color-text-muted)] text-[14px] sm:inline">arrow_forward</span>
                  <div className="relative w-full min-w-0">
                    <input type="text" value={selectedModel} onChange={(e) => setSelectedModel(e.target.value)} placeholder="cc/claude-opus-4-7" className="w-full min-w-0 pl-2 pr-7 py-2 bg-[var(--color-surface)] rounded border border-[var(--color-border)] text-xs focus:outline-none focus:ring-1 focus:ring-primary/50 sm:py-1.5" />
                  ...
                  <button onClick={() => setModalOpen(true)} disabled={!hasActiveProviders} className={`w-full sm:w-auto rounded border px-2 py-2 text-xs transition-colors sm:py-1.5 whitespace-nowrap sm:shrink-0 ${hasActiveProviders ? "bg-[var(--color-surface)] border-[var(--color-border)] text-[var(--color-text-main)] hover:border-[var(--color-text-main)] cursor-pointer" : "opacity-50 cursor-not-allowed border-[var(--color-border)]"}`}>Select</button>
                </div>

                {/* Usage hint */}
                <div className="flex flex-col gap-1 p-3 bg-[var(--color-accent)]/5 border border-[var(--color-accent)]/20 rounded-lg">
                  <p className="text-xs font-medium text-[var(--color-accent)] dark:text-[var(--color-accent)]">Usage:</p>
                  <code className="text-xs font-mono text-[var(--color-text-muted)]">jcode --provider-profile 9router</code>
                  <code className="text-xs font-mono text-[var(--color-text-muted)]">jcode --provider-profile 9router --model {selectedModel || "cc/claude-opus-4-7"}</code>
                </div>
              </div>
