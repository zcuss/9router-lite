              ...
              <div className="flex flex-col gap-3 p-4 bg-[var(--color-accent)]/10 border border-[var(--color-accent)]/30 rounded-lg">
                <div className="flex items-start gap-3">
                  ...
                  <div className="flex-1">
                    <p className="font-medium text-[var(--color-accent)] dark:text-[var(--color-accent)]">Claude CLI not detected locally</p>
                ...
                <div className="p-4 bg-[var(--color-surface)] border border-[var(--color-border)] rounded-lg">
                  ...
                  <div className="space-y-3 text-sm">
                    <div>
                      ...
                      <code className="block px-3 py-2 bg-[var(--color-surface-2)] dark:bg-[var(--color-surface)]/5 rounded font-mono text-xs">npm install -g @anthropic-ai/claude-code</code>
                    </div>
                    <p className="text-[var(--color-text-muted)]">After installation, run <code className="px-1 bg-[var(--color-surface-2)] dark:bg-[var(--color-surface)]/5 rounded">claude</code> to verify.</p>
                  </div>
                </div>
              )}
            </div>
          ...
          {!checkingClaude && claudeStatus?.installed && (
            <>
              <div className="flex flex-col gap-2">
                ...
                {tool.defaultModels.map((model) => (
                  <div key={model.alias} className="grid grid-cols-1 gap-1.5 sm:grid-cols-[8rem_auto_1fr_auto] sm:items-center sm:gap-2">
                    ...
                    <div className="relative w-full min-w-0">
                      <input type="text" value={modelMappings[model.alias] || ""} onChange={(e) => onModelMappingChange(model.alias, e.target.value)} placeholder="provider/model-id" className="w-full min-w-0 pl-2 pr-7 py-2 bg-[var(--color-surface)] rounded border border-[var(--color-border)] text-xs focus:outline-none focus:ring-1 focus:ring-primary/50 sm:py-1.5" />
                      {modelMappings[model.alias] && <button onClick={() => onModelMappingChange(model.alias, "")} className="absolute right-1 top-1/2 -translate-y-1/2 p-0.5 text-[var(--color-text-muted)] hover:text-[var(--color-danger)] rounded transition-colors" title="Clear"><span className="material-symbols-outlined text-[14px]">close</span></button>}
                    </div>
                ...
                {/* CC Filter Naming */}
                <div className="grid grid-cols-1 gap-1.5 sm:grid-cols-[8rem_auto_1fr_auto] sm:items-center sm:gap-2">
                  <span className="text-xs font-semibold text-[var(--color-text-main)] sm:text-right sm:text-sm">Filter naming</span>
                  <span className="material-symbols-outlined hidden text-[var(--color-text-muted)] text-[14px] sm:inline">arrow_forward</span>
                  <label className="flex items-center gap-1.5 cursor-pointer select-none">
                    ...
                    <span className="text-xs text-[var(--color-text-muted)]">Filter naming requests</span>
                  </label>
