              ...
              <div className="flex flex-col gap-3 p-4 bg-[var(--color-accent)]/10 border border-[var(--color-accent)]/30 rounded-lg">
                <div className="flex items-start gap-3">
                  ...
                  <div className="flex-1">
                    <p className="font-medium text-[var(--color-accent)] dark:text-[var(--color-accent)]">OpenCode CLI not detected locally</p>
                ...
                <div className="p-4 bg-[var(--color-surface)] border border-[var(--color-border)] rounded-lg">
                  ...
                  <div className="space-y-3 text-sm">
                    <div>
                      ...
                      <code className="block px-3 py-2 bg-[var(--color-surface-2)] dark:bg-[var(--color-surface)]/5 rounded font-mono text-xs">npm install -g opencode-ai</code>
                    </div>
                    <p className="text-[var(--color-text-muted)]">After installation, run <code className="px-1 bg-[var(--color-surface-2)] dark:bg-[var(--color-surface)]/5 rounded">opencode</code> to verify.</p>
                  </div>
                </div>
              )}
            </div>
            ...
            <>
              <div className="flex flex-col gap-2">
                ...
                <div className="grid grid-cols-1 gap-1.5 sm:grid-cols-[8rem_auto_1fr] sm:items-start sm:gap-2">
                  <span className="w-32 shrink-0 text-sm font-semibold text-[var(--color-text-main)] text-right pt-1">Models</span>
                  ...
                  <div className="flex-1 flex flex-col gap-2">
                        ...
                        selectedModels.map((model) => (
                          <span
                            ...
                            className={`inline-flex items-center gap-1 px-2 py-0.5 rounded text-xs cursor-pointer transition-colors ${
                              model === activeModel
                                ? "bg-[var(--color-text-main)]/10 text-[var(--color-text-main)] border border-[var(--color-text-main)]"
                                : "bg-[var(--color-surface-2)] dark:bg-[var(--color-surface)]/5 text-[var(--color-text-muted)] border border-transparent hover:border-[var(--color-border)]"
                            }`}
                    ...
                    <div className="grid grid-cols-1 gap-1.5 sm:grid-cols-[8rem_auto_1fr_auto] sm:items-center sm:gap-2">
                      <button onClick={() => setModalOpen(true)} disabled={!activeProviders?.length} className={`px-2 py-1 rounded border text-xs transition-colors ${activeProviders?.length ? "bg-[var(--color-surface)] border-[var(--color-border)] text-[var(--color-text-main)] hover:border-[var(--color-text-main)] cursor-pointer" : "opacity-50 cursor-not-allowed border-[var(--color-border)]"}`}>Add Model</button>
                      <span className="text-xs text-[var(--color-text-muted)]">
                        {selectedModels.length > 0 && activeModel ? (
                          <>Active: <span className="text-[var(--color-text-main)]">{activeModel}</span></>
                        ) : selectedModels.length > 0 ? (
                          <span className="text-[var(--color-accent)]">Click a model to set/clear active</span>
                        ) : (
