      ...
      <Card padding="xs" className="overflow-hidden">
        <div className="flex items-start justify-between gap-3 hover:cursor-pointer sm:items-center" onClick={onToggle}>
          <div className="flex min-w-0 items-center gap-3">
            ...
            <div className="min-w-0">
              ...
              <p className="text-xs text-[var(--color-text-muted)] sm:truncate">Intercept {tool.name} requests via MITM proxy</p>
            </div>
          </div>
          <span className={`material-symbols-outlined text-[var(--color-text-muted)] text-[20px] transition-transform ${isExpanded ? "rotate-180" : ""}`}>
            expand_more
          </span>
        </div>
        ...
        {isExpanded && (
          <div className="mt-4 pt-4 border-t border-[var(--color-border)] flex flex-col gap-4">
            {/* Hosts */}
            {mitmHosts.length > 0 && (
              <div className="mt-2 rounded-md border border-[var(--color-border)] bg-[var(--color-surface)]/50 px-2 py-1.5">
                <p className="text-[10px] font-medium tracking-wide text-[var(--color-text-main)]/80 mb-1">
                  Edit hosts file manually to add the following entries:
                </p>
                <ul className="list-none space-y-0.5 font-mono text-[10px] text-[var(--color-text-muted)] break-all">
                  {mitmHosts.map((h) => (
            ...
            {/* Info */}
            <div className="flex flex-col gap-0.5 text-[11px] text-[var(--color-text-muted)] px-1">
              <p>Toggle DNS to redirect {tool.name} traffic through 9Router via MITM.</p>
              {!dnsActive && (
                <p className="text-[var(--color-accent)] text-[10px] mt-1">
              ...
              <div className="flex flex-col gap-2">
                {tool.defaultModels.map((model) => (
                  <div key={model.alias} className="grid grid-cols-1 gap-1.5 sm:grid-cols-[9rem_auto_1fr_auto] sm:items-center sm:gap-2">
                    <span className="text-xs font-semibold text-[var(--color-text-main)] sm:text-right">{model.name}</span>
                    ...
                    <div className="relative w-full min-w-0">
                      <input
                        ...
                        disabled={!dnsActive}
                        className={`w-full min-w-0 pl-2 pr-7 py-2 bg-[var(--color-surface)] rounded border border-[var(--color-border)] text-xs focus:outline-none focus:ring-1 focus:ring-primary/50 sm:py-1.5 ${!dnsActive ? "opacity-50 cursor-not-allowed" : ""}`}
                      />
                    ...
                    <button
                      ...
                      className={`rounded border px-2 py-2 text-xs transition-colors sm:py-1.5 ${hasActiveProviders && dnsActive ? "bg-[var(--color-surface)] border-[var(--color-border)] hover:border-[var(--color-text-main)] cursor-pointer" : "opacity-50 cursor-not-allowed border-[var(--color-border)]"}`}
                    >
                      Select
                    </button>
                  </div>
            ...
            {tool.defaultModels?.length === 0 && (
              <p className="text-xs text-[var(--color-text-muted)] px-1">Model mappings will be available soon.</p>
            )}
            ...
            <div className="flex flex-col gap-2 sm:items-start">
              {dnsActive ? (
                <button
                  ...
                  className="flex w-full items-center justify-center gap-1.5 rounded-lg border border-[var(--color-danger)]/30 bg-[var(--color-danger)]/10 px-4 py-2 text-xs font-medium text-[var(--color-danger)] transition-colors hover:bg-[var(--color-danger)]/20 disabled:cursor-not-allowed disabled:opacity-50 sm:w-auto sm:py-1.5"
                >
                ...
                <button
                  onClick={handleDnsToggle}
                  disabled={!serverRunning || loading}
                  className="flex w-full items-center justify-center gap-1.5 rounded-lg border border-[var(--color-text-main)]/30 bg-[var(--color-text-main)]/10 px-4 py-2 text-xs font-medium text-[var(--color-text-main)] transition-colors hover:bg-[var(--color-text-main)]/20 disabled:cursor-not-allowed disabled:opacity-50 sm:w-auto sm:py-1.5"
                >
              ...
              {warning && (
                <div className="flex items-center gap-2 px-2 py-1.5 rounded text-xs text-[var(--color-accent)]">
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-[var(--color-surface-2)]/50 backdrop-blur-sm">
          <div className="mx-4 flex w-full max-w-sm flex-col gap-4 rounded-xl border border-[var(--color-border)] bg-[var(--color-surface)] p-5 shadow-xl sm:p-6">
            ...
            <div className="flex items-start gap-3 p-3 bg-[var(--color-accent)]/10 border border-[var(--color-accent)]/30 rounded-lg">
              ...
              <p className="text-xs text-[var(--color-text-muted)]">Required to modify /etc/hosts and flush DNS cache</p>
            </div>
            <Input
              type="password"
