    ...
    <Card className="overflow-hidden">
      ...
      <div className="overflow-x-auto">
        <table className="w-full text-sm text-left">
          <thead className="bg-[var(--color-surface)]/30 text-[var(--color-text-muted)] uppercase text-xs">
            <tr>
              {columns.map((col) => (
                <th
                  key={col.field}
                  className={`px-6 py-3 cursor-pointer hover:bg-[var(--color-surface)]/50 ${col.align === "right" ? "text-right" : ""}`}
              ...
              {valueColumns.map((col) => (
                <th
                  key={col.field}
                  className="px-6 py-3 text-right cursor-pointer hover:bg-[var(--color-surface)]/50"
                  onClick={() => onToggleSort(tableType, col.field)}
                >
                  {col.label}{" "}
                  <SortIcon field={col.field} currentSort={sortBy} currentOrder={sortOrder} />
          ...
          <tbody className="divide-y divide-[var(--color-border)]">
            {groupedData.map((group) => (
              <Fragment key={group.groupKey}>
                {/* Group summary row */}
                <tr
                  className="group-summary cursor-pointer hover:bg-[var(--color-surface)]/50 transition-colors"
                ...
                >
                  <td className="px-6 py-3">
                    <div className="flex items-center gap-2">
                      ...
                      <span className={`font-medium transition-colors ${group.summary.pending > 0 ? "text-[var(--color-text-main)]" : ""}`}>
                        {group.groupKey}
                      </span>
                    </div>
                  </td>
                ...
                {expanded.has(group.groupKey) && group.items.map((item) => (
                  <tr
                    key={`detail-${item.key}`}
                    className="group-detail hover:bg-[var(--color-surface)]/20 transition-colors"
                  >
