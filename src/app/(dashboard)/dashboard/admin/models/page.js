  ...
  return (
    <div className="px-4 sm:px-6 py-6 sm:py-8 pb-24 lg:pb-8 max-w-5xl mx-auto space-y-6">
      <header className="flex items-end justify-between flex-wrap gap-3">
        <div>
          <div className="text-[11px] uppercase tracking-wider text-[var(--color-text-subtle)]">Controls</div>
      ...
      <section className="rounded-lg border border-[var(--color-border-subtle)] bg-[var(--color-surface)] p-5 space-y-4">
        <div className="relative max-w-md">
          ...
          <input
            ...
            className="w-full h-10 pl-9 pr-3 rounded-md border border-[var(--color-border-subtle)] bg-transparent text-[13px] outline-none focus:border-[var(--color-text-main)]"
          />
        </div>

        {loading ? (
          <div className="text-[12px] text-[var(--color-text-subtle)]">Loading models…</div>
        ) : (
                ...
                return (
                  <div key={id} className="px-4 py-3 flex items-center gap-3 justify-between">
                    <div className="min-w-0">
                      ...
                      <div className="text-[10px] text-[var(--color-text-subtle)] font-mono truncate">{id}</div>
                    </div>
                    <button
                      onClick={() => toggle(item)}
                      disabled={busy}
                      className={`h-8 px-3 rounded-md text-[11px] font-medium flex items-center gap-1.5 ${
                        enabled
                          ? "bg-lime-300 text-[var(--color-text-main)] hover:bg-lime-200"
                          : "border border-[var(--color-border-subtle)] hover:border-[var(--color-text-main)]"
                      } disabled:opacity-50`}
                    >
