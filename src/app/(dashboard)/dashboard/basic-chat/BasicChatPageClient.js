...
export default function BasicChatPageClient() {
  ...
  return (
    <div className="relative flex-1 flex flex-col h-full min-h-0 min-w-0 bg-[#212121] text-[var(--color-text-main)] overflow-hidden">
      <div className="relative mx-auto flex flex-1 h-full min-h-0 w-full max-w-4xl flex-col">
        <div className="flex shrink-0 items-center justify-between gap-3 px-4 py-3 lg:px-6">
          <div ref={modelMenuRef} className="relative">
            <button
              ...
              className="flex items-center gap-3 rounded-2xl border border-[var(--color-border-subtle)]/10 bg-[var(--color-surface)]/5 px-4 py-3 text-left transition hover:bg-[var(--color-surface)]/8"
            >
              <div className="min-w-0">
                <div className="flex items-center gap-2">
                  <span className="text-sm font-semibold text-[var(--color-text-main)]">{modelLabel}</span>
                  <span className="material-symbols-outlined text-[18px] text-[var(--color-text-main)]/70">expand_more</span>
                </div>
                <p className="truncate text-xs text-[var(--color-text-main)]/55">{modelSubLabel}</p>
              </div>
            </button>

            {modelMenuOpen ? (
              <div className="absolute left-0 top-[calc(100%+10px)] z-30 w-[min(520px,calc(100vw-2rem))] overflow-hidden rounded-[20px] border border-[var(--color-border-subtle)]/10 bg-[#262626] shadow-2xl shadow-[var(--color-shadow)]/50">
                <div className="border-b border-[var(--color-border-subtle)]/10 px-4 py-3">
                  <p className="text-xs uppercase tracking-[0.22em] text-[var(--color-text-main)]/45">Models</p>
                  <p className="text-sm text-[var(--color-text-main)]/75">Only from connected providers</p>
                </div>
                <div className="max-h-[60vh] overflow-y-auto p-2 custom-scrollbar">
                  {providerGroups.map((group) => (
                    <div key={group.providerId} className="mb-2 rounded-[16px] border border-[var(--color-border-subtle)]/10 bg-[var(--color-surface-2)]/20 p-2">
                      <div className="flex items-center justify-between px-2 py-2">
                        <p className="text-sm font-semibold text-[var(--color-text-main)]">{group.providerName}</p>
                        ...
                        {group.models.map((model) => {
                          ...
                          return (
                            <button
                              ...
                              className={`rounded-[14px] border px-3 py-3 text-left transition ${isActive ? "border-[var(--color-accent)]/40 bg-[var(--color-accent)]/15" : "border-[var(--color-border-subtle)]/10 bg-[var(--color-surface)]/5 hover:bg-[var(--color-surface)]/8"}`}
                            >
                              <div className="flex items-start justify-between gap-3">
                                <div className="min-w-0">
                                  <p className="truncate text-sm font-medium text-[var(--color-text-main)]">{model.name}</p>
                                  <p className="truncate text-[11px] text-[var(--color-text-main)]/45">{model.requestModel}</p>
                                </div>
                                {isActive ? <span className="material-symbols-outlined text-[18px] text-[var(--color-accent)]">check_circle</span> : null}
                              </div>
                            </button>
                          );
                        })}
          ...
          <div className="flex items-center gap-2">
            <button
              ...
              className="rounded-2xl border border-[var(--color-border-subtle)]/10 bg-[var(--color-surface)]/5 px-4 py-3 text-sm text-[var(--color-text-main)]/80 transition hover:bg-[var(--color-surface)]/8"
            >
              History
            </button>
        ...
        {historyOpen ? (
          <div ref={historyMenuRef} className="absolute right-4 top-[72px] z-20 w-[min(360px,calc(100vw-2rem))] rounded-[20px] border border-[var(--color-border-subtle)]/10 bg-[#262626] p-2 shadow-2xl shadow-[var(--color-shadow)]/50 lg:right-6">
            <div className="px-3 py-2">
              <p className="text-xs uppercase tracking-[0.22em] text-[var(--color-text-main)]/45">Recent chats</p>
            </div>
            <div className="max-h-[48vh] space-y-2 overflow-y-auto p-1 custom-scrollbar">
              {sessionItems.length === 0 ? (
                <div className="rounded-[16px] border border-dashed border-[var(--color-border-subtle)]/10 bg-[var(--color-surface)]/5 p-4 text-sm text-[var(--color-text-main)]/55">
                ...
                return (
                  <button
                    ...
                    className={`w-full rounded-[16px] border px-3 py-3 text-left transition ${isActive ? "border-[var(--color-accent)]/40 bg-[var(--color-accent)]/15" : "border-[var(--color-border-subtle)]/10 bg-[var(--color-surface)]/5 hover:bg-[var(--color-surface)]/8"}`}
                  >
                    <div className="flex items-start justify-between gap-3">
                      <div className="min-w-0 flex-1">
                        <p className="truncate text-sm font-medium text-[var(--color-text-main)]">{session.title}</p>
                        <p className="mt-1 truncate text-xs text-[var(--color-text-main)]/50">{textValue(latestMessage?.content) || "Empty chat"}</p>
                      </div>
                      <span className="text-[10px] text-[var(--color-text-main)]/40 shrink-0">{formatRelativeTime(session.updatedAt)}</span>
                    </div>
                  </button>
                );
              })}
        ...
        {loadError ? (
          <div className="mt-4 rounded-[18px] border border-[var(--color-danger)]/20 bg-[var(--color-danger)]/10 px-4 py-3 text-[var(--color-danger)]">
        ...
        <div className="flex flex-1 flex-col min-h-0">
            ...
            {currentMessages.length === 0 ? (
              <div className="flex min-h-[50vh] items-center justify-center px-4 text-center">
                <div className="max-w-xl space-y-4">
                  <div className="mx-auto flex size-16 items-center justify-center rounded-[20px] border border-[var(--color-border-subtle)]/10 bg-[var(--color-surface)]/5 text-[var(--color-text-main)]/80">
                    <span className="material-symbols-outlined text-[30px]">chat</span>
                  </div>
                  <div className="space-y-2">
                    <h2 className="text-2xl font-semibold text-[var(--color-text-main)]">Start a conversation</h2>
                    <p className="text-sm leading-6 text-[var(--color-text-main)]/60">
              ...
              {currentMessages.map((message) => {
                ...
                return (
                  <div key={message.id} className={`flex w-full ${isUser ? "justify-end" : "justify-start"} mb-6`}>
                    <div className={`max-w-[min(88%,42rem)] ${isUser ? "rounded-3xl bg-[#2f2f2f] px-5 py-3.5 text-[var(--color-text-main)]" : "text-[var(--color-text-main)]/90"}`}>
                      ...
                      {message.attachments?.length ? (
                        <div className="mb-3 grid grid-cols-2 gap-2 sm:grid-cols-3 mt-2">
                          {message.attachments.map((attachment) => (
                            <a key={attachment.id} href={attachment.dataUrl} target="_blank" rel="noreferrer" className="overflow-hidden rounded-[18px] border border-[var(--color-border-subtle)]/10 bg-[var(--color-surface-2)]/20">
          ...
          <div className="shrink-0 pt-2">
            {attachments.length > 0 ? (
              <div className="mx-auto mb-3 flex w-full max-w-3xl flex-wrap gap-2 px-4">
                {attachments.map((attachment) => (
                  <div key={attachment.id} className="flex items-center gap-2 rounded-full border border-[var(--color-border-subtle)]/10 bg-[var(--color-surface)]/5 px-3 py-2">
                    <span className="text-xs text-[var(--color-text-main)]/80 max-w-[12rem] truncate">{attachment.name}</span>
                    <button type="button" onClick={() => removeAttachment(attachment.id)} className="text-[var(--color-text-main)]/55 hover:text-[var(--color-text-main)]" aria-label="Remove attachment">
            ...
            <div className="mx-auto w-full max-w-3xl px-4 pb-2">
              <div className="rounded-[26px] bg-[#2f2f2f] px-3 pt-3 pb-2 shadow-[0_0_15px_rgba(0,0,0,0.10)] ring-1 ring-[var(--color-border-subtle)]/5">
                <textarea
                  value={draft}
                  ...
                  rows={1}
                  className="w-full resize-none bg-transparent px-2 text-[15px] leading-6 text-[var(--color-text-main)] outline-none placeholder:text-[var(--color-text-main)]/40 custom-scrollbar max-h-[25vh] overflow-y-auto"
                />

                <div className="mt-2 flex items-center justify-between gap-3">
                  <div className="flex items-center gap-2">
                    <button type="button" onClick={() => fileInputRef.current?.click()} disabled={!activeModel || loadingData} className="p-2 text-[var(--color-text-main)]/50 hover:text-[var(--color-text-main)] transition rounded-full hover:bg-[var(--color-surface)]">
                      <span className="material-symbols-outlined text-[20px]">attach_file</span>
                    </button>
                    <input ref={fileInputRef} type="file" accept="image/*" multiple className="hidden" onChange={handleAttachFiles} />
                    <span className="text-xs font-medium text-[var(--color-text-main)]/30 truncate max-w-[120px]">{activeModel ? activeModel.name : "No model"}</span>
                  </div>

                  <div className="flex items-center gap-2">
                    {isSending ? (
                      <button type="button" onClick={handleStop} className="p-2 text-[var(--color-text-main)] bg-[var(--color-surface)]/10 hover:bg-[var(--color-surface)]/20 transition rounded-full h-8 w-8 flex items-center justify-center">
                        <span className="material-symbols-outlined text-[16px]">stop</span>
                      </button>
                    ) : null}
                    <button onClick={sendMessage} disabled={!canSend} className={`h-8 w-8 rounded-full flex items-center justify-center transition ${canSend ? 'bg-[var(--color-surface)] text-[var(--color-text-main)] hover:opacity-90' : 'bg-[var(--color-surface)]/10 text-[var(--color-text-main)]/30 cursor-not-allowed'}`}>
          ...
          <p className="mx-auto mt-2 max-w-3xl px-4 pb-4 text-center text-[11px] text-[var(--color-text-main)]/30">
