    ...
    <div className="mx-auto flex w-full max-w-5xl flex-col gap-4 px-1 sm:gap-6 sm:px-0">
      <div className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
        ...
        <div className="grid grid-cols-1 gap-2 sm:flex sm:items-center">
          <div className="relative" ref={relayMenuRef}>
            ...
            {showRelayMenu && (
              <div className="absolute left-0 top-full z-50 mt-1 w-48 rounded-xl border border-[var(--color-border)]/10 bg-[var(--color-surface)] p-1 shadow-xl dark:border-[var(--color-border-subtle)]/10 dark:bg-[var(--color-text-muted)] sm:left-auto sm:right-0">
                <button
                  onClick={() => {
                    openCloudflareModal();
                    setShowRelayMenu(false);
                  }}
                  className="flex w-full items-center gap-2 rounded-lg px-3 py-2 text-sm text-[var(--color-text-main)] transition-colors hover:bg-[var(--color-surface-2)] dark:hover:bg-[var(--color-surface)]"
                >
                  <span className="material-symbols-outlined text-[20px] text-[var(--color-accent)]">cloud</span>
                  Cloudflare Relay
                </button>
                <button
                  onClick={() => {
                  ...
                  className="flex w-full items-center gap-2 rounded-lg px-3 py-2 text-sm text-[var(--color-text-main)] transition-colors hover:bg-[var(--color-surface-2)] dark:hover:bg-[var(--color-surface)]"
                >
                  <span className="material-symbols-outlined text-[20px] text-[var(--color-accent)]">cloud_upload</span>
                  Vercel Relay
                </button>
                <button
                  onClick={() => {
                  ...
                  className="flex w-full items-center gap-2 rounded-lg px-3 py-2 text-sm text-[var(--color-text-main)] transition-colors hover:bg-[var(--color-surface-2)] dark:hover:bg-[var(--color-surface)]"
                >
                  <span className="material-symbols-outlined text-[20px] text-[var(--color-success)]">terminal</span>
                  Deno Relay
                </button>
              </div>
            )}
      ...
      <Card>
          ...
          {proxyPools.length > 0 && (
            <label className="flex items-center gap-1.5 text-xs text-[var(--color-text-muted)] cursor-pointer">
              <input
                ...
                className="size-4 rounded border-[var(--color-border)]/20 dark:border-[var(--color-border-subtle)]/20"
              />
        ...
        {(selectedIds.length > 0 || healthChecking) && (
          <div className="mb-4 flex flex-wrap items-center gap-2 rounded-lg border border-[var(--color-text-main)]/30 bg-[var(--color-text-main)]/5 px-3 py-2">
            <span className="material-symbols-outlined text-[18px] text-[var(--color-text-main)]">checklist</span>
        ...
        {proxyPools.length === 0 ? (
          ...
          <div className="flex flex-col divide-y divide-[var(--color-border)]/[0.04] dark:divide-[var(--color-border-subtle)]/[0.05]">
            {proxyPools.map((pool) => (
              <div key={pool.id} className="flex flex-col gap-3 py-3 sm:flex-row sm:items-center sm:justify-between">
                <div className="flex items-start gap-3 min-w-0 flex-1">
                  <input
                    ...
                    className="mt-1 size-4 shrink-0 rounded border-[var(--color-border)]/20 dark:border-[var(--color-border-subtle)]/20"
                  />
                  ...
                  <p className="text-xs text-[var(--color-text-muted)] truncate mt-1">{pool.proxyUrl}</p>
                  {pool.noProxy ? (
                    <p className="text-xs text-[var(--color-text-muted)] truncate">No proxy: {pool.noProxy}</p>
                  ) : null}
                ...
                <div className="flex items-center justify-end gap-1">
                  ...
                  <button
                    ...
                    className="p-2 rounded hover:bg-[var(--color-surface-2)] dark:hover:bg-[var(--color-surface)] text-[var(--color-text-muted)] hover:text-[var(--color-text-main)]"
                    title="Test proxy"
      ...
      >
        <div className="flex flex-col gap-4">
          <div>
            <label className="text-sm font-medium text-[var(--color-text-main)] mb-1 block">Paste Proxy List (One per line)</label>
            <textarea
              value={batchImportText}
              onChange={(e) => setBatchImportText(e.target.value)}
              placeholder={"http://user:pass@127.0.0.1:7897\n127.0.0.1:7897:user:pass"}
              className="w-full min-h-[180px] py-2 px-3 text-sm text-[var(--color-text-main)] bg-[var(--color-surface)] dark:bg-[var(--color-surface)]/5 border border-[var(--color-border)]/10 dark:border-[var(--color-border-subtle)]/10 rounded-md focus:ring-1 focus:ring-primary/30 focus:border-[var(--color-text-main)]/50 focus:outline-none transition-all"
            />
      ...
      >
        <div className="flex flex-col gap-4">
          <div className="rounded-lg bg-[var(--color-accent)]/5 border border-[var(--color-accent)]/10 p-3 flex flex-col gap-1.5">
            <p className="text-sm text-[var(--color-text-main)] font-medium">What is Vercel Relay?</p>
          ...
          <Input
            ...
            hint={<>Token is used once for deployment and not stored. <a href="https://vercel.com/account/tokens" target="_blank" rel="noopener noreferrer" className="text-[var(--color-text-main)] hover:underline">Get token →</a></>}
            type="password"
          />
          <Input
            label="Project Name"
      ...
      >
        <div className="flex flex-col gap-4">
          <div className="rounded-lg bg-[var(--color-accent)]/5 border border-[var(--color-accent)]/10 p-3 flex flex-col gap-1.5">
            <p className="text-sm text-[var(--color-text-main)] font-medium">What is Cloudflare Relay?</p>
            <p className="text-xs text-[var(--color-text-muted)]">
            ...
            <div className="mt-2 pt-2 border-t border-[var(--color-accent)]/10 text-xs text-[var(--color-text-muted)]">
              <p className="font-medium text-[var(--color-text-main)] mb-1">How to generate your API Token:</p>
          ...
          <Input
            ...
            hint={<>Requires "Workers Scripts: Edit" permission. <a href="https://dash.cloudflare.com/profile/api-tokens" target="_blank" rel="noopener noreferrer" className="text-[var(--color-text-main)] hover:underline">Get token →</a></>}
            type="password"
          />
          <Input
            label="Worker Name"
      ...
      >
        <div className="flex flex-col gap-4">
          <div className="rounded-lg bg-[var(--color-surface-2)] dark:bg-[var(--color-surface)]/5 border border-[var(--color-border)]/10 dark:border-[var(--color-border-subtle)]/10 p-3 flex flex-col gap-1.5">
            <p className="text-sm text-[var(--color-text-main)] font-medium">What is Deno Relay?</p>
            <p className="text-xs text-[var(--color-text-muted)]">
            ...
            <div className="mt-2 pt-2 border-t border-[var(--color-border)]/10 dark:border-[var(--color-border-subtle)]/10 text-xs text-[var(--color-text-muted)]">
              <p className="font-medium text-[var(--color-text-main)] mb-1">How to generate API token:</p>
              <ol className="list-decimal pl-4 space-y-0.5">
      ...
      >
        <div className="flex flex-col gap-4">
          ...
          <div className="flex flex-col gap-3 rounded-lg border border-[var(--color-border)]/50 p-3 sm:flex-row sm:items-center sm:justify-between">
            <div>
              <p className="font-medium text-sm">Active</p>
              <p className="text-xs text-[var(--color-text-muted)]">Inactive pools are ignored by runtime resolution.</p>
            </div>
            <Toggle
          ...
          <div className="flex flex-col gap-3 rounded-lg border border-[var(--color-border)]/50 p-3 sm:flex-row sm:items-center sm:justify-between">
            <div>
              ...
              <p className="text-xs text-[var(--color-text-muted)]">Fail request if proxy is unreachable instead of falling back to direct.</p>
            </div>
            <Toggle
