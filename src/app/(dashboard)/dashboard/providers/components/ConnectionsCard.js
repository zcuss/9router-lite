...
function CooldownTimer({ until }) {
  ...
  return <span className="text-xs text-[var(--color-accent)] font-mono">⏱ {remaining}</span>;
}
...
function ConnectionRow({ connection, proxyPools, isOAuth, isFirst, isLast, onMoveUp, onMoveDown, onToggleActive, onUpdateProxy, onEdit, onDelete }) {
  ...
  return (
    <div className={`group flex flex-col gap-3 p-2 rounded-lg sm:flex-row sm:items-center sm:justify-between hover:bg-[var(--color-surface-2)] dark:hover:bg-[var(--color-surface)]/[0.02] transition-colors ${connection.isActive === false ? "opacity-60" : ""}`}>
      <div className="flex w-full min-w-0 flex-1 items-start gap-3 sm:items-center">
        <div className="flex flex-col">
          <button onClick={onMoveUp} disabled={isFirst} className={`p-0.5 rounded ${isFirst ? "text-[var(--color-text-muted)]/30 cursor-not-allowed" : "hover:bg-sidebar text-[var(--color-text-muted)] hover:text-[var(--color-text-main)]"}`}>
            <span className="material-symbols-outlined text-sm">keyboard_arrow_up</span>
          </button>
          <button onClick={onMoveDown} disabled={isLast} className={`p-0.5 rounded ${isLast ? "text-[var(--color-text-muted)]/30 cursor-not-allowed" : "hover:bg-sidebar text-[var(--color-text-muted)] hover:text-[var(--color-text-main)]"}`}>
            <span className="material-symbols-outlined text-sm">keyboard_arrow_down</span>
          </button>
        </div>
        <span className="material-symbols-outlined text-base text-[var(--color-text-muted)]">{isOAuth ? "lock" : "key"}</span>
        <div className="flex-1 min-w-0">
          ...
          <div className="flex flex-wrap items-center gap-2 mt-1">
            ...
            {connection.lastError && connection.isActive !== false && (
              <span className="text-xs text-[var(--color-danger)] truncate max-w-[300px]" title={connection.lastError}>{connection.lastError}</span>
            )}
            <span className="text-xs text-[var(--color-text-muted)]">#{connection.priority}</span>
          </div>
          {hasAnyProxy && (
            <div className="mt-1 flex flex-wrap items-center gap-2">
              <span className="text-[11px] text-[var(--color-text-muted)] truncate max-w-[420px]" title={proxyDisplayText}>{proxyDisplayText}</span>
              {maskedProxyUrl && <code className="text-[10px] font-mono bg-[var(--color-surface-2)] dark:bg-[var(--color-surface)]/5 px-1 py-0.5 rounded text-[var(--color-text-muted)]">{maskedProxyUrl}</code>}
              {noProxyText && <span className="text-[11px] text-[var(--color-text-muted)] truncate max-w-[320px]" title={noProxyText}>no_proxy: {noProxyText}</span>}
            </div>
          )}
        </div>
      </div>
          ...
          {(proxyPools || []).length > 0 && (
            <div className="relative" ref={proxyDropdownRef}>
              <button
                ...
                className={`flex flex-col items-center px-2 py-1 rounded hover:bg-[var(--color-surface-2)] dark:hover:bg-[var(--color-surface)] transition-colors ${hasAnyProxy ? "text-[var(--color-text-main)]" : "text-[var(--color-text-muted)] hover:text-[var(--color-text-main)]"}`}
              ...
              {showProxyDropdown && (
                <div className="absolute right-0 top-full mt-1 z-50 bg-bg border border-[var(--color-border)] rounded-lg shadow-lg py-1 min-w-[160px]">
                  <button onClick={() => handleSelectProxy("__none__")} className={`w-full text-left px-3 py-1.5 text-sm hover:bg-[var(--color-surface-2)] dark:hover:bg-[var(--color-surface)] ${!boundProxyPoolId ? "text-[var(--color-text-main)] font-medium" : "text-[var(--color-text-main)]"}`}>None</button>
                  {(proxyPools || []).map((pool) => (
                    <button key={pool.id} onClick={() => handleSelectProxy(pool.id)} className={`w-full text-left px-3 py-1.5 text-sm hover:bg-[var(--color-surface-2)] dark:hover:bg-[var(--color-surface)] ${boundProxyPoolId === pool.id ? "text-[var(--color-text-main)] font-medium" : "text-[var(--color-text-main)]"}`}>{pool.name}</button>
                  ))}
                </div>
              )}
            </div>
          )}
          <button onClick={onEdit} className="flex flex-col items-center px-2 py-1 rounded hover:bg-[var(--color-surface-2)] dark:hover:bg-[var(--color-surface)] text-[var(--color-text-muted)] hover:text-[var(--color-text-main)]">
    ...
    <Modal isOpen={isOpen} title={`Add ${providerName || provider} API Key`} onClose={onClose}>
      <div className="flex flex-col gap-4">
        <div>
          ...
          <input className="w-full px-3 py-2 text-sm border border-[var(--color-border)] rounded-lg bg-background focus:outline-none focus:border-[var(--color-text-main)]" value={formData.name} onChange={(e) => setFormData({ ...formData, name: e.target.value })} placeholder="Production Key" />
        </div>
        <div className="flex gap-2">
          <div className="flex-1">
            <label className="text-xs text-[var(--color-text-muted)] mb-1 block">API Key</label>
            <input type="password" className="w-full px-3 py-2 text-sm border border-[var(--color-border)] rounded-lg bg-background focus:outline-none focus:border-[var(--color-text-main)]" value={formData.apiKey} onChange={(e) => setFormData({ ...formData, apiKey: e.target.value })} />
          </div>
          <div className="pt-6">
        ...
        <div>
          ...
          <input type="number" className="w-full px-3 py-2 text-sm border border-[var(--color-border)] rounded-lg bg-background focus:outline-none focus:border-[var(--color-text-main)]" value={formData.priority} onChange={(e) => setFormData({ ...formData, priority: Number.parseInt(e.target.value) || 1 })} />
        </div>
...
export default function ConnectionsCard({ providerId, isOAuth }) {
  ...
  if (loading) return <Card><div className="h-20 animate-pulse bg-[var(--color-surface-2)] rounded-lg" /></Card>;
            ...
            {providerStrategy === "round-robin" && (
              <div className="flex flex-wrap items-center gap-1.5">
                ...
                <input
                  ...
                  className="w-16 px-2 py-1 text-xs border border-[var(--color-border)] rounded-md bg-background focus:outline-none focus:border-[var(--color-text-main)]"
                />
              </div>
            )}
          </div>
