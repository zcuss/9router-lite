...
export function ModelRow({ model, fullModel, copied, onCopy, testStatus, isCustom, isFree, onDeleteAlias, onTest, isTesting }) {
  const borderColor = testStatus === "ok" ? "border-[var(--color-success)]/40" : testStatus === "error" ? "border-[var(--color-danger)]/40" : "border-[var(--color-border)]";
      ...
      <div className="flex items-center gap-2">
        ...
        {onTest && (
          <div className="relative group/btn">
            <button onClick={onTest} disabled={isTesting} className={`p-0.5 hover:bg-sidebar rounded text-[var(--color-text-muted)] hover:text-[var(--color-text-main)] transition-opacity ${isTesting ? "opacity-100" : "opacity-0 group-hover:opacity-100"}`}>
        ...
        <div className="relative group/btn">
          <button onClick={() => onCopy(fullModel, `model-${model.id}`)} className="p-0.5 hover:bg-sidebar rounded text-[var(--color-text-muted)] hover:text-[var(--color-text-main)]">
        ...
        {isFree && <span className="text-[10px] font-bold text-[var(--color-success)] bg-[var(--color-success)]/10 px-1.5 py-0.5 rounded">FREE</span>}
        {isCustom && (
          <button onClick={onDeleteAlias} className="p-0.5 hover:bg-[var(--color-danger)]/10 rounded text-[var(--color-text-muted)] hover:text-[var(--color-danger)] opacity-0 group-hover:opacity-100 transition-opacity ml-auto" title="Remove custom model">
  ...
  return (
    <Modal isOpen={isOpen} title="Add Custom Model" onClose={onClose}>
      <div className="flex flex-col gap-4">
        <div>
          <label className="text-xs text-[var(--color-text-muted)] mb-1 block">Model ID</label>
          <input
  ...
  return (
    <>
      <Card>
        ...
        {testError && <p className="text-xs text-[var(--color-danger)] mb-3 break-words">{testError}</p>}
        ...
        <div className="flex flex-wrap gap-3">
          ...
          <button
            ...
            className="flex items-center gap-1.5 px-3 py-2 rounded-lg border border-dashed border-[var(--color-border)]/15 dark:border-[var(--color-border-subtle)]/15 text-xs text-[var(--color-text-muted)] hover:text-[var(--color-text-main)] hover:border-[var(--color-text-main)]/40 transition-colors"
          >
