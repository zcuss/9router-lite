...
const endpointUrl = typeof window !== "undefined" ? `${window.location.origin}/api/v1` : "/api/v1";
  ...
  return (
    <div className="px-4 sm:px-6 py-6 sm:py-8 pb-24 lg:pb-8 max-w-5xl mx-auto space-y-6">
      <header className="flex items-end justify-between flex-wrap gap-3">
        <div>
          <div className="text-[11px] uppercase tracking-wider text-text-subtle">Access</div>
          <h1 className="text-2xl sm:text-3xl font-bold tracking-tight mt-1">Endpoint & keys</h1>
        </div>
        <button
          onClick={() => setShowCreate(true)}
          className="h-9 px-4 rounded-md bg-text-main text-surface text-[12px] font-medium flex items-center gap-2"
        >
      ...
      <section className="rounded-lg border border-border-subtle bg-surface p-5">
        <div className="flex items-center gap-2 mb-3">
          <Link2 size={14} className="text-text-subtle" />
          <span className="text-[10px] uppercase tracking-wider text-text-subtle font-medium">Base endpoint</span>
        </div>
        <div className="flex items-center gap-2 flex-wrap">
          <code className="flex-1 min-w-[260px] rounded-md border border-border-subtle bg-surface-2 px-3 py-2 text-[12px] font-mono text-text-main overflow-x-auto">
            {endpointUrl}
          </code>
          <button
            onClick={() => copyText(endpointUrl, "endpoint")}
            className="h-9 px-3 rounded-md border border-border-subtle text-[12px] font-medium hover:border-text-main flex items-center gap-2"
          >
      ...
      <section className="rounded-lg border border-border-subtle bg-surface overflow-hidden">
        <div className="px-5 py-4 border-b border-border-subtle flex items-center justify-between">
          ...
          <span className="text-[11px] text-text-subtle">{keys.length} total</span>
        </div>
              ...
              return (
                <div key={key.id} className="px-5 py-3.5 flex items-center gap-3">
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <div className="text-[10px] text-text-subtle font-mono truncate">{visible ? raw : masked}</div>
                  </div>
      ...
      <section className="rounded-lg border border-border-subtle bg-surface p-5">
        <div className="text-[10px] uppercase tracking-wider text-text-subtle font-medium mb-3">Quick start</div>
        <div className="rounded-md border border-border-subtle bg-surface-2 p-4 overflow-x-auto">
          <pre className="text-[11px] font-mono leading-6 text-text-main whitespace-pre-wrap">{`from openai import OpenAI
