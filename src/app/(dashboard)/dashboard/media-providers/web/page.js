...
function ProviderCard({ provider, kind, connections }) {
  ...
  const renderStatus = () => {
    ...
    if (total === 0) return <span className="text-xs text-[var(--color-text-muted)]">No connections</span>;
    return (
      <>
  ...
  return (
    <Link href={`/dashboard/media-providers/${kind}/${provider.id}`} className="group">
      <Card padding="xs" className={`h-full hover:bg-[var(--color-surface-2)] dark:hover:bg-[var(--color-surface)]/[0.01] transition-colors cursor-pointer ${allDisabled ? "opacity-50" : ""}`}>
...
function ComboList({ combos }) {
  if (combos.length === 0) {
    return <p className="text-xs text-[var(--color-text-muted)] italic">No combos yet.</p>;
  }
  return (
    <div className="flex flex-col gap-2">
      {combos.map((combo) => (
        <Link key={combo.id} href={`/dashboard/media-providers/combo/${combo.id}`}>
          <Card padding="xs" className="hover:bg-[var(--color-surface-2)] dark:hover:bg-[var(--color-surface)]/[0.02] transition-colors cursor-pointer">
            <div className="flex min-w-0 items-center gap-3">
              ...
              <div className="flex flex-wrap items-center gap-1 sm:shrink-0">
                ...
                {combo.models.length > 6 && (
                  <span className="text-[10px] text-[var(--color-text-muted)] ml-1">+{combo.models.length - 6}</span>
                )}
              </div>
              <span className="text-[11px] text-[var(--color-text-muted)] shrink-0">{combo.models.length}</span>
              <span className="material-symbols-outlined text-[var(--color-text-muted)] text-[16px]">chevron_right</span>
            </div>
          </Card>
        </Link>
      ))}
    ...
    <div>
      ...
      <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between mb-3">
        <div className="flex flex-wrap items-center gap-2">
          <span className="material-symbols-outlined text-[var(--color-text-main)]">{icon}</span>
          <h2 className="text-base font-semibold">{title}</h2>
          <span className="text-xs text-[var(--color-text-muted)]">({providers.length} providers · {combos.length} combos)</span>
        </div>
      ...
      {providers.length === 0 ? (
        <div className="text-center py-8 border border-dashed border-[var(--color-border)] rounded-xl text-[var(--color-text-muted)] text-sm">
          No providers.
        </div>
      ) : (
