      ...
      <div className="flex flex-col gap-4">
          ...
          <>
            ...
            <div className="grid grid-cols-2 gap-3">
              <button
                ...
                className="flex flex-col items-center gap-2 p-4 rounded-lg border border-[var(--color-border)] hover:border-[var(--color-text-main)] hover:bg-[var(--color-text-main)]/5 transition-colors text-left"
              >
                <span className="material-symbols-outlined text-2xl text-[var(--color-text-main)]">lock_open</span>
                <div>
                  <p className="text-sm font-medium">OAuth App</p>
                  <p className="text-xs text-[var(--color-text-muted)]">Use a GitLab OAuth application</p>
                </div>
              </button>
              <button
                onClick={() => setMode("pat")}
                className="flex flex-col items-center gap-2 p-4 rounded-lg border border-[var(--color-border)] hover:border-[var(--color-text-main)] hover:bg-[var(--color-text-main)]/5 transition-colors text-left"
              >
                <span className="material-symbols-outlined text-2xl text-[var(--color-text-main)]">key</span>
                <div>
                  <p className="text-sm font-medium">Personal Access Token</p>
                  <p className="text-xs text-[var(--color-text-muted)]">Use a GitLab PAT with api scope</p>
                </div>
              </button>
            </div>
          </>
        ...
        {mode === "oauth" && (
          <>
            <p className="text-xs text-[var(--color-text-muted)]">
              ...
              <a href={`${baseUrl.trim() || GITLAB_COM}/-/profile/applications`} target="_blank" rel="noreferrer" className="text-[var(--color-text-main)] underline">
                GitLab Applications
              </a>{" "}
              with redirect URI{" "}
        ...
        {mode === "pat" && (
          <>
            <p className="text-xs text-[var(--color-text-muted)]">
              Create a PAT at{" "}
              <a href={`${baseUrl.trim() || GITLAB_COM}/-/user_settings/personal_access_tokens`} target="_blank" rel="noreferrer" className="text-[var(--color-text-main)] underline">
