...
export async function exportDb() {
  ...
  const out = {
    ...
    apiKeys: (await db.all(`SELECT * FROM apiKeys`)).map((r) => ({ id: r.id, key: r.key, name: r.name, machineId: r.machineId, isActive: r.isActive === 1 || r.isActive === true, createdAt: r.createdAt })),
...
export async function importDb(payload) {
  ...
  await db.transactionAsync(async () => {
    ...
    for (const [a, m] of Object.entries(payload.modelAliases || {})) {
      await db.run(`INSERT OR REPLACE INTO kv(scope, key, value) VALUES('modelAliases', ?, ?)`, [a, stringifyJson(m)]);
    }
