...
async function syncSchemaFromTables(adapter) {
  for (const [tableName, def] of Object.entries(TABLES)) {
    ...
    // Diff columns. Non-SQLite adapters expose listColumns() because PRAGMA is SQLite-only.
    const existing = typeof adapter.listColumns === "function"
      ? await adapter.listColumns(tableName)
      : await adapter.all(`PRAGMA table_info(${tableName})`);
...
export async function runMigrationOnce(adapter) {
  ...
  if (fresh && hasLegacy && !alreadyImported) {
    ...
    pruneOldBackups();
    console.log(`[DB][migrate] JSON → remote DB in ${Date.now() - t0}ms | legacy JSON kept at DATA_DIR | backup: ${backupDir}`);
    return;
  }
  ...
  // 4. App version bump → backup legacy/local artifacts if present (safety net before user-side upgrade)
  ...
  if (oldVer && oldVer !== newVer) {
    ...
    for (const f of Object.values(LEGACY_FILES)) {
      try { backupFile(f, backupDir); } catch {}
    }
